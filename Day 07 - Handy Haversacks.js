function day07(input) {
  ...
  for (var i = 0; i < input.length; i++) {
    var newRule = input[i];
    var newRuleTokens = newRule.split(" bags contain ");
    var color = newRuleTokens[0];
    if (newRuleTokens[1] !== "no other bags.") {
      var valTokens = newRuleTokens[1].split(", ");
      var values = [];
      for (var j = 0; j < valTokens.length; j++) {
        values.push(valTokens[j].split(" bag")[0]);
      }
      rules[color] = values;
    }
  }
  ...
  for (var key in rules) {
    if (rules.hasOwnProperty(key)) {
      if (key !== "shiny gold" && canContainShinyGold(key, rules)) {
        count++;
      }
    }
  }
  console.log(count + " bag colors can contain shiny gold.");
  // Part 2:
  console.log("A shiny gold bag contains " + (countBags("shiny gold", rules) - 1) + " bags.");
}

function canContainShinyGold(color, rules) {
  if (!rules[color]) {
    return false;
  }
  if (color === "shiny gold") {
  ...
  for (var i = 0; i < rules[color].length; i++) {
    returnValue |= canContainShinyGold(rules[color][i].substr(2), rules);
  }
  return returnValue;
}
...
function countBags(color, rules) {
  ...
  var returnValue = 0;
  for (var i = 0; i < rules[color].length; i++) {
    returnValue += Number(rules[color][i][0]) * countBags(rules[color][i].substr(2), rules);
  }
