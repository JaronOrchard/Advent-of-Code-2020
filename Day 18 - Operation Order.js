...
function day18(input) {
  ...
  for (var i = 0; i < input.length; i++) {
    var question = input[i].replaceAll(" ", "");
    var tokens = [];
    // Assumes only 1-digit per number:
    for (var j = 0; j < question.length; j++) {
      tokens.push(question[j]);
    }
    while (tokens.length > 1) {
      // Do something!
      var index = 0;
      while (true) {
        // If number enclosed by parens, remove parens:
        if (tokens[index] === "(" && tokens[index + 2] === ")") {
          tokens.splice(index + 2, 1);
          tokens.splice(index, 1);
          break;
        }
        // If add and not a parens, add to prev num then remove + and subsequent num:
        if (tokens[index] === "+" && tokens[index + 1] !== "(") {
          tokens[index - 1] = Number(tokens[index - 1]) + Number(tokens[index + 1]);
          tokens.splice(index, 2);
          break;
        }
        // If multiply and not a parens, multiply with prev num then remove * and subsequent num:
        if (tokens[index] === "*" && tokens[index + 1] !== "(") {
          tokens[index - 1] = Number(tokens[index - 1]) * Number(tokens[index + 1]);
          tokens.splice(index, 2);
          break;
        }
        // Wasn't able to do anything, try from next token:
        index++;
      }
    }
    sum += Number(tokens[0]);
  ...
  for (var i = 0; i < input.length; i++) {
    ...
    while (tokens.length > 1) {
      ...
      while (true) {
        ...
        // If multiplication and not a parens -- check first if there are any additions elsewhere in the current capture group.
        // If there are, we have to skip this and come back later.  Otherwise, we can do the multiplication.
        if (tokens[index] === "*" && tokens[index + 1] !== "(") {
          var multiplyOk = true;
          // Look rightward for "+"
          for (var j = index + 1; j < tokens.length; j++) {
            if (tokens[j] === "+" || tokens[j] === "(") {
              ...
              // Ends capture group, you can do whatever you need to within this one.
              break;
            }
          }
          if (multiplyOk) {
