...
function day14(input) {
  ...
  for (var i = 0; i < input.length; i++) {
    if (stringContains(input[i], "mask")) {
      mask = input[i].substr(7);
      continue;
    }
    // Must be "mem":
    var leftSide = input[i].split(" = ")[0];
    var leftNum = leftSide.substr(4, leftSide.length - 5);
    var rightNum = Number(input[i].split(" = ")[1]);
    var rightMask = padTo36(dec2bin(rightNum));
    // Apply mask:
    for (var j = 0; j < rightMask.length; j++) {
      if (mask[j] !== "X") {
        rightMask = rightMask.substr(0, j) + mask[j] + rightMask.substr(j+1);
      }
    }
    memory[leftNum] = rightMask;
  }
  var sum = 0;
  for (var key in memory) {
    if (memory.hasOwnProperty(key)) {
      sum += bin2dec(memory[key]);
    }
  }
  console.log(sum);
}
...
function day14(input) {
  ...
  for (var i = 0; i < input.length; i++) {
    ...
    var leftSide = input[i].split(" = ")[0];
    var leftNum = leftSide.substr(4, leftSide.length - 5);
    var leftMask = padTo36(dec2bin(leftNum))
    var rightNum = Number(input[i].split(" = ")[1]);
    // Apply 1s in mask:
    for (var j = 0; j < leftMask.length; j++) {
      if (mask[j] === "1" || mask[j] === "X") {
        leftMask = replaceCharAt(leftMask, j, mask[j]);
      }
    }
    // Get all possible floating results:
    var floatingSet = [deepCopy(leftMask)];
    while (stringContains(floatingSet[0], "X")) {
      var nextMask = floatingSet[0];
      floatingSet = floatingSet.slice(1);
      var nextXIndex = nextMask.indexOf("X");
      floatingSet.push(replaceCharAt(nextMask, nextXIndex, "0"));
      floatingSet.push(replaceCharAt(nextMask, nextXIndex, "1"));
    }
    for (var j = 0; j < floatingSet.length; j++) {
      memory[floatingSet[j]] = rightNum;
    }
  }
  var sum = 0;
  for (var key in memory) {
